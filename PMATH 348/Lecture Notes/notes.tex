...
g(x) \bar h(x)$ from which we see that $\bar g(x)$ and $h(x)$ have 0 constant
...
\end{example}
\begin{example}
Consider the \underline{cyclotomic polynomial} defined by
\begin{equation*}
...
$p^{\text{th}}$ cyclotomic polynomial. Notice here, since $p$ is coprime with
...
\begin{equation*}
\Phi_p(x+1) = \frac{(x+1)^p -1}{x} = x^p + \binom{p}{1}x^{p-1} \in \Z[x]
\end{equation*}
...
$\Q[x]$, so $\Phi_p(x)$ is irreducible as well. Since $\Phi_p(x)$ is
primitive, $\Phi_p(x)$ is also irreducible in $\Z[x]$.
\end{example}
\end{document}
