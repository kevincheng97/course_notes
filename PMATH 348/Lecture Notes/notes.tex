...
\end{theorem}
\begin{proof}
By \cref{sst}, the number of Sylow $p$-subgroups of $G$ is the number of conjugates
of any one of them, say $P$. This number is $[G:N_G(P)]$ which is a divisor of
$|G|$. Let $S$ be the set of all Sylow $p$-subgroups of $G$ and let $P$ act on
$S$ by conjugation. Then, $Q \in S_0$ if and only if $xQx^{-1} = Q$ for all $x
\in P$. The latter condition holds if and only if $P \subseteq N_G(Q)$. Both $P$
and $Q$ are Sylow $p$-subgroups of $G$ and hence of $N_G(Q)$. Thus, by
\cref{corollary35}, the are conjugate in $N_G(Q)$. Since $Q \triangleleft
N_G(Q)$, this can only occur if $Q=P$. Thus, $S_0 = \{p\}$ and by
...
$p$-subgroups of $G$. By \cref{tst}, we have $n_3 \mid 15$ and $n_3 \equiv 1
\Mod{3}]$. Thus, $n_3 = 1$. Similarly, since $n_5 \mid 15$ and $n_5 \equiv 1
\Mod{5}$, $n_5 = 1$. It follows that there is only one Sylow 3-subgroup and one
Sylow 5-subgroup in $G$, say $P_3$ and $P_5$ respectively. Thus, $P_3
...
\end{example}
Let $G$ be a group of order $21 = 7 \cdot 3$. Let $n_p$ be the number os Sylow
$p$-subgroups of $G$. By \cref{tst}, we have $n_3 \mid 21$ and $n_3 \equiv 1
\Mod{3}$. Thus, $n_3 = 1$ or 7. Similarly, we have $n_7 \mid 21$ and $n_7 \equiv
1 \Mod{7}$. Thus, $n_7 = 1$. It follows that $G$ has a unique Sylow 7-subgroup,
say $P_7$ and note that $P_7 \triangleleft G$ and $P_7$ is cyclic, say
$P_7 = \cyclic{x}$ with $x^7 = 1$. Let $H$ be a Sylow 3-subgroup. Since $|H| =
...
\end{equation*}
which has 21 distinct elements. Here, $G$ is generated by $x$ and $y$.
...
\faktor{\quotient{G}{N}}[1]{\quotient {H}{N}}[-1.5]$.
\label{tit}
\end{theorem}
...
\triangleleft \quotient{G_iN}{N}$. By \cref{tit},
\begin{equation*}
